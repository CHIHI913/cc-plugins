...
description: MarkdownのリーンキャンバスをMiroボード上のテンプレートフレームに付箋として自動配置する。リーンキャンバスの12セクション（カスタマーセグメント/アーリーアダプター/課題/既存の代替品/UVP/ハイレベルコンセプト/ソリューション/チャネル/収益の流れ/コスト構造/主要指標/圧倒的な優位性）をパースし、light_yellow付箋でMiro Bulk Create APIで一括作成する。「/lean-canvas-to-miro」「miroにリーンキャンバスを配置」「リーンキャンバスをmiroに反映」で起動。
---
